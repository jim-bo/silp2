...
def write_agp(paths, args, runtime=None):
    """ translates oriented graph to ag file
    Parameters
    ----------
    paths.gap_file       : file
    args.agp_file           : file
    ...
    SG = nx.read_gpickle(paths.gap_file)
    ...
    # save it to disk.
    to_agp(SG, args.agp_file)

    # append the timing.
    fin = open(args.agp_file, "rb")
    lines = fin.readlines()
    fin.close()
    if runtime != None:
        txt = "RUNTIME:\t%f\n" % (runtime)
...
def write_fasta(paths, args):
    """ translates AGP and contigs to fasta file"""
    
    ctg_file = args.ctg_file
